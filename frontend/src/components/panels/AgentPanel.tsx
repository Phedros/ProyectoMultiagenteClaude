...
export default function AgentPanel() {
  ...
  const startEdit = (agent: Agent) => {
    setForm({ name: agent.name, system_prompt: agent.system_prompt, model: agent.model, temperature: agent.temperature })
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        {agents.map((agent) => (
          ...
          >
            ...
            {expandedId === agent.id && (
              <div className="border-t border-[#2d3148] px-3 py-2">
                <p className="text-xs text-slate-400 leading-relaxed line-clamp-4">{agent.system_prompt}</p>
                <p className="mt-1 text-xs text-slate-500">Temperature: {agent.temperature}</p>
              </div>
            )}
            {/* Drag hint */}
            <div
